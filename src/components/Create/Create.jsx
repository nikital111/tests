...
function Create({ questions, show, deleteQ, editQ }) {
  const list = questions.map((cont, id) => {
    ...
    return (
      <div key={id} className="question">
        <h1>{cont.title}</h1>
        <button className="editButt" onClick={editActionQ}>
          Edit
        </button>
        <button className="delButt" onClick={deleteActionQ}>
          Delete
        </button>
      </div>
    );
  ...
  return (
    <>
      <div className="createCont">{list}</div>
...
const mapDispatchToProps = (dispatch) => ({
  ...
  deleteQ: (id) => {
    dispatch({ type: "DELETE_Q", id: id });
  },
  editQ: (id) => {
    dispatch({ type: "EDIT_Q", id: id });
  },
});
