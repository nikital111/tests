...
const Form = ({ question, idQ, isShow, close, create, editFormQ }) => {
  const [state, setState] = useState(
    idQ !== null ? question : [{ title: "", isCorrect: false }]
  );
  useEffect(() => {
  ...
  const listEF =
    idQ !== null
      ? Object.keys(question.matters).map((q, id) => {
          return (
            <>
              <h1 key={"title" + id}>question {id + 1}</h1>
              <div className="createdQF" key={"div" + id}>
                ...
                <input
                  type="text"
                  defaultValue={question.matters[q].title}
                  id={"t" + id}
                  onChange={stateTitle}
                  key={"text" + id}
                ></input>
  ...
  return (
    <div className="wrap">
      <form className="root">
        ...
        <button className="del" onClick={closeForm}>
          X
        </button>
        ...
        <div className="createdQ">{idQ !== null ? listEF : listF}</div>
        ...
        <div className="contButtForm">
          ...
          <button
            className="create"
            onClick={idQ === null ? createList : editQ}
          >
...
const toState = (dispatch) => ({
  ...
  create: (obj) => {
    dispatch({ type: "CREATE_Q", payload: obj });
  },
  editFormQ: (id, obj) => {
    dispatch({ type: "EDIT_FORM_Q", id: id, obj: obj });
  },
});
