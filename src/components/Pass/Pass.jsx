...
function Pass({questions,sendResult}) {
    const [state,setState] = useState({
       ...
       const list = Object.keys(questions[state.numberOfQ]).map((q,id) => {
           ...
           if(id>0) return null;
           const quest = Object.keys(questions[state.numberOfQ].matters).map((q2,id2) => {

            const setCorrect = (e) => {
                e.preventDefault();
                const butt = document.querySelector('.checkButt');
                butt.removeAttribute('disabled');
                if(questions[state.numberOfQ].matters[q2].isCorrect) setState({...state,isCorrect:true});
            ...
            return( 
                   <li key={id2} className="qq">
                     <input type='radio' name={'question' + state.numberOfQ} id={'radio' + state.numberOfQ} className='answers' onInput={setCorrect}></input>
                     <label htmlFor={'radio' + state.numberOfQ}>{questions[state.numberOfQ].matters[q2].title}</label>
                    </li>
                          )
           })
           return (
  ...
  return (
   <div className='contPass'>
       <div className='progress'>{state.numberOfQ+1}/{questions.length}</div>
        {list}
        <div className='checkCont'>
        <button className='checkButt' onClick={checkAnswer}>Check</button>
       <NavLink exact to='/result'>
       <button className='checkRes' onClick={res} disabled={questions.length-state.numberOfQ === 1 && state.ok ? false : true}>Check u RESULT</button>
       </NavLink>
...
const mapStateToProps = state => {
   ...
   const mapDispatchToProps = dispatch => ({
     sendResult:(rating,max) => {dispatch({type:'RESULT',rating:rating,max:max})},
    });
